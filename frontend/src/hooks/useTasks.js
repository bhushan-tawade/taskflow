...
const useTasks = () => {
  ...
  const createTask = async () => {
    ...
    try {
      ...
      setTitle(""); setDescription(""); setPriority("medium"); setCategory("personal"); setDueDate("");
      fetchTasks();
  ...
  const toggleInProgress = async (task) => {
    try {
      await updateTaskAPI(token, task._id, { status: "in_progress", is_done: false });
  ...
  return {
    ...
    status, setStatus, dueDate, setDueDate,
    fetchTasks, createTask, deleteTask, toggleComplete, toggleInProgress, updateTask,
    totalTasks, completedTasks, pendingTasks, overdueTasks, recentTasks
  };
};
